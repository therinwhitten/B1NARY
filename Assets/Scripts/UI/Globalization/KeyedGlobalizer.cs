	...
	public class KeyedGlobalizer : MonoBehaviour
	{
		...
		public int ExpectedLines
		{
			...
			set
			{
				...
				{
					languageKeys.Add("");
					languageValues.Add(string.Join(SPLIT_CHAR.ToString(), Enumerable.Repeat("", languages.Count)));
				}
				while (languageKeys.Count > value)
				{
		...
		public List<LanguageValuePair> this[string key]
		{
			...
			set
			{
				...
				LinkedList<string> existingLanguages = new LinkedList<string>(languages);
				...
				List<LanguageValuePair> sortedList = new List<LanguageValuePair>(value.Count);
				...
				for (int i = 0; i < languageKeys.Count; i++)
				{
					...
					languageValues[i] = string.Join(SPLIT_CHAR.ToString(), unmerged);
					return;
				}
				throw new IndexOutOfRangeException(key);
			}
		...
		{
			text.text = "";
			oldValue = new LanguageValuePair();
		}


		internal void UpdateLanguageList()
		{
			HashSet<string> existingLanguage = new HashSet<string>(languages);
			...
			HashSet<string> preExistingLanguages = new HashSet<string>(Languages.Instance);
			for (int i = 0; i < languages.Count; i++)
